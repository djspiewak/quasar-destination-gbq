...
object GBQDestinationModuleSpec extends EffectfulQSpec[IO] {
  ...
  //TODO: fix this, read json file and store the bytes
  //TODO: fix this test, don't actually use a real json service account file
  ...
  val aDataset = "mydatasettest"

  "fail gbq destination with bad token and bad project" >>* {
    val destination = Resource.suspend(configWith(badToken, badProject, aDataset).map(GBQDestinationModule.destination[IO](_)))
    destination.use(dst => IO(dst must beLeft(accessDeniedError)))
  }

  "fail gbq destination with bad token and good project" >>* {
    val destination = Resource.suspend(configWith(badToken, goodProject, aDataset).map(GBQDestinationModule.destination[IO](_)))
    destination.use(dst => IO(dst must beLeft(accessDeniedError)))
  }

  "fail gbq destination with good token and bad project" >>* {
    val destination = Resource.suspend(configWith(goodToken, badProject, aDataset).map(GBQDestinationModule.destination[IO](_)))
    destination.use(dst => IO(dst must beLeft(invalidConfig(badProject))))
  }

  "succeed when checking gbq destination with good token and good project" >>* {
    val destination = Resource.suspend(configWith(goodToken, goodProject, aDataset).map(GBQDestinationModule.destination[IO](_)))
    destination.use(dst => IO(dst must beRight))
  }

  def configWith(token: String, project: String, datasetId: String): IO[Json] =
    IO(Json.obj(
        "datasetId" := datasetId,
        "project" := project,
        "token" := token))
